...
class btn(disnake.ui.View):
    ...
    async def interaction_check(self, interaction: MessageInteraction):
        ...
        button = disnake.ui.Button(label="Перейти в комнату", style=disnake.ButtonStyle.url, url='...')
    ...
    @disnake.ui.button(emoji="👁️", style=disnake.ButtonStyle.gray, custom_id="hide_unhide")
    ...
    @disnake.ui.button(emoji="🔒", style=disnake.ButtonStyle.gray, custom_id="lock")
    ...
    @disnake.ui.button(emoji="🔓", style=disnake.ButtonStyle.gray, custom_id="unlock")
    ...
    @disnake.ui.button(emoji="✏️", style=disnake.ButtonStyle.gray, custom_id="rename")
    async def rename(self, button: disnake.Button, inter: disnake.CommandInteraction):
        ...
        if inter.author.id in self.last_button_use:
            ...
            if time_difference.total_seconds() < 300:
                оставшееся_время = datetime.timedelta(seconds=300 - time_difference.total_seconds())
    ...
    @disnake.ui.button(emoji="🔇", style=disnake.ButtonStyle.gray, custom_id="mute")
    ...
    @disnake.ui.button(emoji="🔊", style=disnake.ButtonStyle.gray, custom_id="unmute")
    ...
    @disnake.ui.button(emoji="🔑", style=disnake.ButtonStyle.gray, custom_id="locktoall")
    ...
    @disnake.ui.button(emoji="🚪", style=disnake.ButtonStyle.gray, custom_id="kick")
    ...
    @disnake.ui.button(emoji="🛠️", style=disnake.ButtonStyle.gray, custom_id="limit")
    ...
    @disnake.ui.button(emoji="👑", style=disnake.ButtonStyle.gray, custom_id="crown")
...
class CreateCog(commands.Cog):
    ...
    async def s(self, ctx):
        ...
        embed = disnake.Embed(title="Настройка приватных комнат",
                              description=
                              "Жми следующие кнопки, чтобы настроить свою комнату\n"
                              "Использовать их можно только когда у тебя есть приватный канал\n\n"
                              "👁️ — `Скрыть комнату от всех`\n"
                              "🔒 — `Забрать доступ в комнату`\n"
                              "🔓 — `Выдать доступ в комнату`\n"
                              "✏️ — `Изменить название канала`\n"
                              ":mute: — `Выключить микрофон участнику`\n"
                              "🔊 — `Включить микрофон участнику`\n"
                              "🔑 — `Закрыть комнату от всех`\n"
                              ":door: — `Выгнать участника`\n"
                              ":tools: — `Установить лимит пользователей`\n"
                              "👑 — `Назначить пользователя владельцем комнаты\n`",
                              color=0x36393f)
